...
fuzz_target!(|input: TypedFuzzInstructionPrototype| {
    ...
    // logging
    {
        ...
        let log_msg = format!("{:?}:\n - cpu_instruction_cost: {}\n - memory_bytes_cost: {}", fuzz_instruction, cpu_instruction_cost, memory_bytes_cost);

        let mut log_file = OpenOptions::new().write(true).create(true).append(true).open("budget_log.txt").unwrap();
        writeln!(&mut log_file, "{}\n{}", log_msg, env.budget()).unwrap();
    }
